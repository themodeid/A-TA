...
export const uploadAbsensi = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  if (!req.file) {
    throw new AppError("File tidak ditemukan, mohon unggah file Excel", 400);
  }

  const idPeriodeRaw = req.body.id_periode || req.body.idPeriode || req.query.id_periode;
  if (!idPeriodeRaw) {
