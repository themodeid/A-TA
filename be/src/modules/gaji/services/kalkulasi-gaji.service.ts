...
export class KalkulasiGajiService {
  constructor() {
    // Constructor dikosongkan karena pool menggunakan shared instance hasil import
  }

  /**
   * Service untuk melakukan kalkulasi gaji akhir berdasarkan ID Periode
   */
  async kalkulasiGajiAkhir(
    idPeriode: number,
    ...
    try {
      ...
      // Catatan: Variabel vRateLembur bisa digunakan jika dibutuhkan nanti
      ...
      // =========================================================================
      // C. LOOPING KALKULASI DI MEMORI NODE.JS
      // =========================================================================
      for (const pgw of listPegawai) {
        ...
        // Simpan Tunjangan Striktural (jika ada)
        if (tunjStruktural > 0) {
          await client.query(
            ...
            [
              idRekap,
              `Tunjangan Struktural ${pgw.nama_jabatan}`,
              tunjStruktural,
            ],
          );
        }

        // Simpan Tunjangan Variabel
      ...
      console.error("Gagal kalkulasi gaji, transaksi dibatalkan:", error);
      throw error;
    } finally {
