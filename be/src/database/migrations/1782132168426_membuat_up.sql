...
-- ==========================================
-- Ganti 'public' di bawah ini dengan nama schema lo jika lo tidak pakai schema default
CREATE OR REPLACE FUNCTION public.fungsi_kalkulasi_gaji_akhir(p_id_periode INT) 
RETURNS VOID AS $$
DECLARE
...
BEGIN
    ...
    FROM public.tb_tunjangan WHERE kode_kondisi = 'LEMBUR_PER_JAM' AND deleted_at IS NULL;

    -- =========================================================================
    -- 2. INISIALISASI WADAH DATA (ANTI-ZONK & DATA-DRIVEN UPDATE)
    -- =========================================================================
    
    -- A. Pastikan baris Rekap Gaji & Snapshot Pegawai sudah ada
    INSERT INTO public.tb_rekap_gaji (id_periode, id_pegawai, jabatan_snapshot, pangkat_golongan_snapshot, gaji_pokok_snapshot)
    SELECT 
        p_id_periode, 
        p.id_pegawai, 
    ...
    FROM public.tb_absensi_summary abs
    ...
    FROM public.tb_absensi_summary abs
    CROSS JOIN public.tb_master_potongan mp
    ...
    FROM public.tb_absensi_summary abs
    CROSS JOIN public.tb_tunjangan t
    WHERE abs.id_periode = p_id_periode 
    ...
    SET nilai_terhitung = CASE t.formula_type
            ...
            WHEN 'PER_JAM_LEMBUR' THEN tb.total_jam_lebih * t.nilai
    ...
    -- =========================================================================
    UPDATE public.tb_potongan_bulanan pb
    SET total_potongan_terhitung = COALESCE((
        SELECT SUM(nilai_potongan) 
        FROM public.tb_potongan_bulanan_detail pbd 
    ...
    -- =========================================================================
    UPDATE public.tb_rekap_gaji rg
    SET 
        total_penghasilan_bruto = rg.gaji_pokok_snapshot 
            + COALESCE((SELECT j.tunjangan_jabatan_struktural FROM public.tb_jabatan j JOIN public.tb_pegawai p ON p.id_jabatan = j.id_jabatan WHERE p.id_pegawai = rg.id_pegawai), 0)
            + COALESCE((SELECT SUM(nilai_terhitung) FROM public.tb_tunjangan_bulanan_detail WHERE id_periode = p_id_periode AND id_pegawai = rg.id_pegawai), 0)
            + COALESCE((SELECT honor_bulan FROM public.tb_tunjangan_bulanan WHERE id_periode = p_id_periode AND id_pegawai = rg.id_pegawai), 0),
        
        total_potongan = COALESCE((SELECT total_potongan_terhitung FROM public.tb_potongan_bulanan WHERE id_periode = p_id_periode AND id_pegawai = rg.id_pegawai), 0)
    ...
    JOIN public.tb_pegawai p ON p.id_pegawai = rg.id_pegawai
    ...
    SELECT 
        ...
        'TUNJANGAN', 
        CASE 
            WHEN t.formula_type = 'PER_JAM_LEMBUR' THEN 'Honor Lembur (' || tb.total_jam_lebih || ' Jam)'
            ELSE t.nama_tunjangan 
        END, 
        td.nilai_terhitung, 
        t.kode_kondisi
    FROM public.tb_rekap_gaji rg
    ...
    FROM public.tb_rekap_gaji rg
    JOIN public.tb_potongan_bulanan_detail pbd ON rg.id_pegawai = pbd.id_pegawai AND rg.id_periode = pbd.id_periode
    JOIN public.tb_master_potongan mp ON pbd.id_master_potongan = mp.id_master_potongan
